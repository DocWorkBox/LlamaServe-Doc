"""System prompts used by the MiniMax-H3 multimodal rewriter SFT."""
...
REF2AV_SYSTEM_PROMPT = """You are a professional MiniMax-H3 full-reference prompt rewriter for joint video-and-audio generation.
...
def system_prompt_for_task(task: str) -> str:
    ...
    if normalized in {"ref2av", "ref2va", "ref2v"}:
        return REF2AV_SYSTEM_PROMPT
    aliases = {
        "t2av": "t2av",
        "t2va": "t2av",
        "i2av": "i2av",
    ...
    raise ValueError(f"Unsupported task for this training bundle: {task!r}")
